...
export default function AdminSinsPage() {
    ...
    return (
        ...
        <CardHeader className="flex flex-row justify-between items-start">
            <div>
            <CardTitle className="text-2xl font-headline flex items-center gap-2"><AlertTriangle/>إدارة المهلكات</CardTitle>
            <CardDescription>
                ...
                <TableRow key={item.id}>
                    <TableCell>
                        {item.icon.startsWith('http') ? (
                            <Image src={item.icon} alt={item.title} width={40} height={40} className="rounded-md" />
                        ) : (
                            <span className="text-muted-foreground text-xs">{item.icon}</span>
                        )}
                    </TableCell>
                    <TableCell className="font-medium">{item.title}</TableCell>
                    <TableCell className="hidden md:table-cell">{item.dialogTitle}</TableCell>
