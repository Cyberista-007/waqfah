  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-start mb-4">
        <div>
          <h1 className="text-4xl font-bold mb-4 font-headline">{lecture.title}</h1>
          <p className="text-muted-foreground text-lg mb-6">
            من <Link href={`/series/${lecture.seriesSlug}`} className="text-primary-foreground/80 hover:underline">{lecture.seriesTitle}</Link>
          </p>
        </div>
        <Button variant="ghost" size="icon" className="p-2 rounded-full bg-card shadow">
          <Heart className="w-8 h-8 text-gray-400 hover:text-red-500 hover:fill-current" />
      ...
      <section>
        <h3 className="text-2xl font-semibold mb-4 font-headline">روابط التحميل</h3>
        <div className="flex flex-wrap gap-4">
          <Button asChild className="bg-primary/80 text-primary-foreground hover:bg-primary/90">
       ...
       <section>
        <h3 className="text-2xl font-semibold mb-4 font-headline">مشاركة المحاضرة</h3>
        <div className="flex flex-wrap gap-4">
          <Button className="bg-blue-600 text-white hover:bg-blue-700 w-full sm:w-auto"><Facebook className="me-2"/><span>فيسبوك</span></Button>
          <Button className="bg-sky-500 text-white hover:bg-sky-600 w-full sm:w-auto"><BrandIcon brand="twitter"/><span className="ms-2">تويتر</span></Button>
          <Button className="bg-green-500 text-white hover:bg-green-600 w-full sm:w-auto"><BrandIcon brand="whatsapp"/><span className="ms-2">واتساب</span></Button>
        </div>
      </section>
      ...
      <section>
        <h3 className="text-2xl font-semibold mb-4 font-headline">تقييم المحاضرة</h3>
        <Card>
            <CardContent className="p-6">
                <div className="flex items-center gap-2 text-yellow-400">
                    <p className="text-lg font-bold text-foreground">{lecture.rating}</p>
                    {[...Array(5)].map((_, i) => (
                        <Star key={i} className={`w-6 h-6 ${i < Math.round(lecture.rating) ? 'fill-current' : ''}`} />
                    ))}
                    <p className="text-sm text-muted-foreground ms-2">({lecture.ratingCount} تقييم)</p>
                </div>
                 <p className="text-muted-foreground mt-2">
                    يرجى <Link href="/auth/login" className="text-primary-foreground/80 hover:underline">تسجيل الدخول</Link> لإضافة تقييمك.
                </p>
            </CardContent>
        </Card>
      </section>
      ...
      <section>
        <h3 className="text-2xl font-semibold mb-4 font-headline">التعليقات</h3>
        <Card>
            <CardContent className="p-6">
                <div className="mb-6">
