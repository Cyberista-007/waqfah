...
import { Sparkles, Trophy, Award, Lock, CheckCircle2, Headphones, BookOpen, Film, Gem, Star, Loader2, ArrowRight, ArrowLeft, Search, Filter } from 'lucide-react';
...
export default function BadgesPage() {
    ...
    const handleBadgeClick = (earned: boolean) => {
        if (earned) {
            confetti({
                particleCount: 80,
                spread: 60,
    ...
    if (isLoading) {
        return (
            <div className="flex h-screen flex-col items-center justify-center bg-[#050505] space-y-4">
    ...
    return (
        <div className="min-h-screen pb-32 overflow-hidden bg-[#050505] text-right" dir="rtl">
            {/* 🎭 Background Aura */}
            <div className="fixed inset-0 pointer-events-none overflow-hidden">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[600px] bg-primary/5 blur-[150px] rounded-full" />
                <div className="absolute bottom-0 right-0 w-[500px] h-[500px] bg-amber-500/5 blur-[120px] rounded-full translate-x-1/4 translate-y-1/4" />
            </div>
            ...
            <div className="container relative z-10 px-4 max-w-6xl mx-auto">
                ...
                <header className="pt-20 pb-12 flex flex-col md:flex-row justify-between items-center gap-8 border-b border-white/5">
                    ...
                    {user ? (
                        ...
                        >
                            <div className="flex items-center gap-4">
                                ...
                                <div className="space-y-0.5">
                                    <h3 className="font-black text-white text-lg">{userProfile?.name}</h3>
                                    <p className="text-[10px] font-black uppercase text-primary tracking-widest">{userProfile?.points || 0} نقطة إجمالية</p>
                                </div>
                            </div>
                        ...
                        <div className="w-full md:w-80 p-8 rounded-[2.5rem] bg-white/[0.02] backdrop-blur-3xl border border-white/5 text-center space-y-4">
                                ...
                                {filteredBadges.map((badge) => {
                                    ...
                                    return (
                                        <motion.div
                                            ...
                                            onClick={() => handleBadgeClick(isEarned)}
                                            className={cn(
                                                "group p-6 rounded-[2.2rem] border transition-all duration-500 flex flex-col relative overflow-hidden select-none",
                                                isEarned 
                                                    ? "border-amber-500/20 bg-amber-500/[0.03] shadow-[0_20px_50px_rgba(245,158,11,0.05)] cursor-pointer"
